...
(function( $ ) {
	...
	*/
	...
	var clickedItem1,
		...
		mainClass1: {
			open: function(){
				$('#thumbGalleryDetail1').owlCarousel().trigger('refresh.owl.carousel');
				$('#thumbGalleryDetail1').owlCarousel().trigger('to.owl.carousel', [clickedItem1, 0]);

				$('#thumbGalleryThumbs1').owlCarousel('refresh');

				removeShowThumbsTimeout1 = setTimeout(function(){
					$('#thumbGalleryThumbs1').removeClass('show-thumbs1');
				}, 3000);

				$(document).on('keydown', function( event ) {
				    if(event.keyCode == 37) {
				        $('#thumbGalleryDetail1').trigger('prev.owl')
				    }
				    if(event.keyCode == 39) {
				        $('#thumbGalleryDetail1').trigger('next.owl')
				    }
				});
			...
			close: function(){
				...
				$('#thumbGalleryThumbs1').addClass('show-thumbs1');
	...
	var clickedItem1 = '';
	if( $('a[href="#ourHistoryLightbox1"]').length ) {
		$('a[href="#ourHistoryLightbox1"]').on('click', function(){
			clickedItem1 = $(this).parent().index();
		});

		$('a[href="#ourHistoryLightbox1"]').magnificPopup(portfolioLightboxOptions1);
	}

	/*
	Thumb Gallery
	*/
	if( $('#ourHistoryLightbox1').get(0) ) {
